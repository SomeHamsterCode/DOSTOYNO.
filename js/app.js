 ...
 * Версия 3.0 Final - Исправленный множественный выбор
 */
...
let state = { username: "", coins: 0, xp: 0, level: 1, streak: 0, lastLogin: null, dailyRewardClaimed: false, buildings: {}, cityBalance: 0, inventory: ['hair_default', 'cloth_default', 'acc_none'], equipped: { hair: 'hair_default', clothes: 'cloth_default', acc: 'acc_none' }, progress: {}, achievements: [], soundEnabled: true, notificationsEnabled: true, mapZoom: 1, buildingPositions: {} };

function init() {
    const saved = localStorage.getItem('dostoino_save_v3');
    if (saved) {
        try {
            ...
            localStorage.removeItem('dostoino_save_v3');
        }
    }
}
...
function saveGame() {
    try {
        localStorage.setItem('dostoino_save_v3', JSON.stringify(state));
        updateUI();
    } catch (e) {
...
function startGame() {
    ...
    if (!name) { showToast('⚠️ Введите имя персонажа!'); return; }
    if (name.length < 2) { showToast('⚠️ Имя должно быть не менее 2 символов'); return; }
    state.username = name;
    ...
    saveGame();
    document.getElementById('page-onboarding').style.opacity = '0';
    setTimeout(() => {
        document.getElementById('page-onboarding').style.display = 'none';
        document.getElementById('app-interface').style.display = 'block';
...
function calculateIncome() {
    let income = 0;
    DB.buildings.forEach(b => { if (state.buildings[b.id]) { income += (state.buildings[b.id].count * b.income); } });
...
function updateUI() {
    document.getElementById('coinDisplay').innerText = Math.floor(state.coins);
    document.getElementById('headerName').innerText = state.username;
    document.getElementById('headerLevel').innerText = state.level;
    document.getElementById('headerXP').innerText = state.xp % 100;
    document.getElementById('streakDisplay').innerText = state.streak;
    document.getElementById('profileName').innerText = state.username;
    document.getElementById('profileXp').innerText = state.xp % 100;
    document.getElementById('xpFill').style.width = `${state.xp % 100}%`;
    const income = calculateIncome();
    document.getElementById('cityIncome').innerText = income.toFixed(1) + ' 🪙/сек';
    document.getElementById('cityBalance').innerText = Math.floor(state.cityBalance);
    document.getElementById('cpsDisplay').innerText = income.toFixed(1);
    let icon = '🧑‍🎓';
    if (state.equipped.hair) { const h = DB.shopItems.find(x => x.id === state.equipped.hair); if (h) icon = h.icon; }
    document.getElementById('headerAvatar').innerText = icon;
    document.getElementById('shopAvatarPreview').innerText = icon;
    document.getElementById('profileAvatar').innerText = icon;
    document.getElementById('startAvatar').innerText = icon;
    let bonus = 0;
    ['hair', 'clothes', 'acc'].forEach(type => {
        if (state.equipped[type]) { const item = DB.shopItems.find(x => x.id === state.equipped[type]); if (item) bonus += item.bonus; }
    });
    document.getElementById('avatarBonus').innerText = '+' + bonus.toFixed(1);
}
...
function renderRoadmap() {
    ...
    const svgPath = document.getElementById('roadmapPath');
    container.innerHTML = '';
